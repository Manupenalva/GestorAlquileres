...
export function MonthlyReport({ 
  ...
  const monthPayments = payments.filter(p => p.buildingId === buildingId && p.month === selectedMonth && p.isPaid);
  const monthExpenses = expenses.filter(e => {
    const expenseDate = new Date(e.date);
    return e.buildingId === buildingId && expenseDate.toISOString().slice(0, 7) === selectedMonth;
  });
        ...
        <div className="flex items-start justify-between flex-wrap gap-4 mb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {building.cantidadDepartamentos ?? 0} departamentos · {building.cantidadInquilinos ?? 0} inquilinos · Expensas base ${(building.expensasBase ?? 0).toLocaleString()}
            </p>
          </div>
